...
int main(void)
{
  ...
  groveLCD_term_printf("sonita est belle");


  HAL_Delay(50);
  uint16_t distane_arr ;
 /** while (1)
    { distane_arr = VL53L0X_readRangeContinuousMillimeters();
	  captDistIR_Get(tab_dist);
  	  printf("la distance est %d, %d \r\n", tab_dist[0],tab_dist[1]);
  	 printf("la distance arriere  est %d \r\n", distane_arr);
  	  HAL_Delay(1000);
